...
5. $c(x - z, b(x - z), c(x - z), b(x - z))$ 循环直到 $x=0$
   ...
   void sortstack(stack<int>& s) {
       int deep = getdeep(s);
       while(deep > 0) {
           int maxv = stackmax(s, deep);
           int times = maxcount(s, deep, maxv);
           down(s, deep, maxv ,times);
           deep -=times;
       }
   } 
   ```
...
```cpp
void hannota(int i, int from, int to, int other) {
    if(i == 1) {
        cout<<i << from << to;
    }
    if(i != 1){
        hannota(i-1, from, other, to);
        cout<< i << from << to;
        hannota(i-1, other, to, from);
    }
}
```
